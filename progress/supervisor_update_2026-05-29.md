...
- An unweighted BERT control suggests this is mainly a dataset-shift problem, not only caused by the class-weighted loss.
- Intermediate fine-tuning on a small amount of FakeNewsNet target-domain training data substantially improves the held-out target test result in the current seed-42 pass. The best current setting in this pass is 20% target-domain fine-tuning, with Macro-F1 0.7447 on the held-out FakeNewsNet test split.
