...
- The in-domain LIAR setting reaches about 0.61-0.62 Macro-F1, showing that the current DeepSeek V4 Flash atom setup contains some usable signal in this sample.
- The LIAR -> FakeNewsNet transfer setting drops sharply in Macro-F1, especially for Logistic Regression, so the current setup performs only modestly under cross-dataset transfer.
...
- This supports the narrower dissertation argument that the current DeepSeek V4 Flash + 1000-row atom setup does not remove cross-dataset generalisation difficulty; domain and style shift still matter.

## Dissertation Positioning

Treat this experiment as an exploratory pilot inspired by TELLER's separation between cognition-style signal generation and decision aggregation. It should not be framed as a reproduced TELLER system or as the dissertation's main contribution. Its role is to provide a small supporting analysis showing that the current DeepSeek V4 Flash atom setup contains some signal but has limited performance under the current cross-dataset setting.
